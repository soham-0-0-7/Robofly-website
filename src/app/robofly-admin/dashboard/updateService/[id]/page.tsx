...
export default function UpdateServicePage() {
  ...
  useEffect(() => {
    const checkSessionAndFetchService = async () => {
      try {
        ...
        if (!sessionData.user.permissions?.service?.updateServices) {
          setSessionError(
            "You don't have permission to update services. Redirecting to dashboard..."
          );
        ...
        if (!serviceResponse.ok) {
          setSessionError(`Failed to fetch service data: ${serviceData.error}`);
          setTimeout(() => router.push("/robofly-admin/dashboard"), 2000);
          return;
        }
        ...
        setFormData({
          ...
          mainImage: serviceData.service.mainImage,
        });
      } catch (error) {
        console.error("Error:", error);
        setSessionError(
          "Failed to verify your session or fetch service data. Redirecting to login..."
        );
        setTimeout(() => router.push("/robofly-admin"), 2000);
      } finally {
        setLoading(false);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <LoadingSpinner text="Loading service data..." />
      </div>
    );
  }
          ...
          <div className="bg-white shadow-md rounded-lg overflow-hidden">
            <div className="px-6 py-4 bg-purple-50 border-b">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Update details for service ID: {serviceId}
              </p>
            </div>
