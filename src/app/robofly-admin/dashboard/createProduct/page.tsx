  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        if (!data.user.permissions?.product?.addProducts) {
          setSessionError(
            "You don't have permission to create products. Redirecting to dashboard..."
          );
          setTimeout(() => router.push("/robofly-admin/dashboard"), 2000);
          return;
        }

        // Get highest product ID for default value
        const productsResponse = await fetch("/api/products/getAll");
        const productsData = await productsResponse.json();

        if (productsResponse.ok && productsData.products?.length > 0) {
          const highestId = Math.max(
            ...productsData.products.map((product: any) => product.id)
          );
